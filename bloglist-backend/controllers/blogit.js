...
blogitRouter.post('/:id/comments', async (request, response) => {
  try {
    let blog = await Blogi.findById(request.params.id)
    const body = request.body
    console.log('TÄMÄ ON BODY', body)
    ...
    response.json(formatBlogi(commentedBlog))
  } catch (exception) {
    console.log(exception)
    response.status(500).json({ error: 'pilalla' })
  }
